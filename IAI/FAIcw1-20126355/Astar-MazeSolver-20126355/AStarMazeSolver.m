...
for i = 1 : MAX_X
    for j = 1 : MAX_Y
        if(maze(i, j) == 0 || maze(i, j) == 8)               %% -1 to 0, 8
            OBSTACLE(k, 1) = i;
            OBSTACLE(k, 2) = j;
            k = k + 1;
        end
        if(maze(i, j) == 3)                         %% get start point
            OBSTACLE(k, 1) = i;
            OBSTACLE(k, 2) = j;
            k = k + 1;
            xStart = i - 1;
...
maze(xStart, yStart) = 5;

%% Start the search
while((xNode ~= xTarget || yNode ~= yTarget) && NoPath == 1)
    dispMaze(maze);
    % expand the current node to obtain child nodes
    exp = expend_mazeSolver(xNode, yNode, path_cost, xTarget, yTarget, OBSTACLE, MAX_X, MAX_Y);
    ...
    if (index_min_node ~= -1)
        ...
        maze(xNode,yNode) = 5;                            %% choose path
    else
